...
tools = new Env('Test')
 ...
 // 发通知
tools.msg(tools.name, "sub_titile", "点击跳转到Github", "https://github.com")

// 写持久化变量
tools.setdata("value-666", "this_is_a_key");

// 读持久化变量
var result = tools.getdata("this_is_a_key");
...
tools.get(test_get, (error, response, data) =>{
    if(error){
        tools.log(error)
    }
    tools.log(data)
...
tools.post(test_post, (error, response, data) =>{
    if(error){
        tools.log(error)
    }
    tools.log(data)
